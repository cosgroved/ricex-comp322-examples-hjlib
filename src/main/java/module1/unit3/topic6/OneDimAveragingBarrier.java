...
/**
 * <p>OneDimAveragingBarrier class.</p>
 *
...
public class OneDimAveragingBarrier {
    ...
    /**
     * <p>Constructor for OneDimAveragingBarrier.</p>
     *
     * @param n a int.
     */
    public OneDimAveragingBarrier(final int n) {
        this.n = n;
    ...
    public static void main(final String[] args) {
...

        {
            ...
            final OneDimAveragingBarrier initialObj = new OneDimAveragingBarrier(n);
        ...
        launchHabaneroApp(() -> {
            for (int r = 0; r < rounds; r++) {
                final OneDimAveragingBarrier serialBody = new OneDimAveragingBarrier(n);
                final OneDimAveragingBarrier forallBodyGrouped = new OneDimAveragingBarrier(n);
                final OneDimAveragingBarrier forallBodyBarrier = new OneDimAveragingBarrier(n);
                ...
                timeIt(String.format("Forall-barrier [tasks=%d]", tasks), () -> {
                    forallBodyBarrier.runForallBarrier(iterations, tasks);
                }, () -> {
                    forallBodyBarrier.validateOutput();
                });
     ...
     * <p>setOutput.</p>
     *
     * @param oneDimAveraging a {@link edu.rice.hj.example.comp322.OneDimAveragingBarrier} object.
     */
    public static void setOutput(final OneDimAveragingBarrier oneDimAveraging) {
        if (OneDimAveragingBarrier.initialOutput != null) {
            System.out.println("Warning: initialOutput has already been set.");
        }
        OneDimAveragingBarrier.initialOutput = oneDimAveraging.myVal;
    }
    ...
    public void validateOutput() {
        ...
        for (int i = 0; i < n + 2; i++) {
            final double init = OneDimAveragingBarrier.initialOutput[i];
    ...
    /**
     * <p>runForallBarrier.</p>
     *
     * @param iterations a int.
     * @param tasks      a int.
     */
    public void runForallBarrier(final int iterations, final int tasks) throws SuspendableException {
        final HjRegion.HjRegion1D iterSpace = newRectangularRegion1D(1, n);
        ...
        forallPhased(0, tasks - 1, (i) -> {
            ...
            for (int iter = 0; iter < iterations; iter++) {
                final double[] myVal = (iter % 2 == 0) ? this.myVal : this.myNew;
                final double[] myNew = (iter % 2 == 0) ? this.myNew : this.myVal;
                forseqNb(myGroup, (j) -> {
                    myNew[j] = (myVal[j - 1] + myVal[j + 1]) / 2.0;
                });
